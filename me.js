...
(function () {
  ...
  /* 캐시 우선 토픽 — 홈 위젯(오늘의 토론·프로필)은 신선도보다 요청 절약이 중요.
     메모리 캐시/로컬 스냅샷이 있으면 서버 왕복 0 · 없으면(첫 방문) 서버 1회. */
  async function topicsCheap() {
    var c = DB.cachedTopics && DB.cachedTopics();
  ...
  /* ── 배지 정의 + 판정 ── */
  var BADGES = [
    { k: 'first', ic: 'ballot', n: '첫 한 표', d: '첫 표 던졌다. 시작이 반이다.' },
    { k: 'ten', ic: 'grid', n: '10토론 참여', d: '토론 10개에 입장 박음.' },
    { k: 'streak7', ic: 'flame', n: '7일 연속', d: '7일 연속 참여. 이쯤이면 중독.' },
    { k: 'minority', ic: 'bolt', n: '소수파 챔피언', d: '다수와 반대편에 다섯 번 이상. 소신 하나는 인정.' },
    { k: 'oracle', ic: 'target', n: '예측왕', d: '예측 적중률 80% 돌파. 판 읽는 눈 있네.' },
    { k: 'like10', ic: 'trophy', n: '공감 10+', d: '내 댓글이 공감 10개 이상 받음.' },
    { k: 'allcat', ic: 'medal', n: '전 카테고리', d: '6개 카테고리 전부 투표 완료.' },
    { k: 'night', ic: 'clock', n: '심야 토론러', d: '새벽(0~5시)에 토론한 적 있음.' }
  ];
  ...
  function unlock(badge) {
    ...
    u.querySelector('.ue').innerHTML = icon(badge.ic, 50);
  ...
  function persistBadges(earned) {
    ...
    if (fresh.length) {
      ...
      fresh.slice(1).forEach(function (k) { var b = BADGES.filter(function (x) { return x.k === k; })[0]; if (b) thjToast(b.n + ' 배지 획득!', 'success'); });
    }
  }
  ...
  async function updateBallotBadge() {
    ...
    // 배지는 부가 정보 — 캐시만 쓰고 서버 요청은 절대 만들지 않는다 (내가 투표한 토론은 스냅샷에 있음)
    var topics = (DB.cachedTopics && DB.cachedTopics()) || [];
    if (!topics.length) { setN(0); return; }
  ...
  async function renderToday() {
    ...
    var topics = await topicsCheap();   // 캐시/스냅샷 우선 — 재방문 시 서버 왕복 0
    ...
    var streakLine = '';
    if (!closed) {
      if (votedToday) streakLine = '<div class="daily-streak done">' + icon('flame', 13) + ' <b>' + streak + '일</b> 연속 · 내일도 이 시간에 새 토론 열린다</div>';
      else if (streak > 0) streakLine = '<div class="daily-streak">' + icon('flame', 13) + ' <b>' + streak + '일</b> 연속 · 오늘 한 표로 <b>' + (streak + 1) + '일째</b></div>';
    }
  ...
  function recapBlock(by) {
    ...
    var ico = icon(hit == null ? 'ballot' : (hit ? 'target' : 'eye'), 20);
  ...
  async function renderProfile() {
    ...
    var v = votes(), ids = Object.keys(v);
    var topics = await topicsCheap(); var by = {}; topics.forEach(function (t) { by[t.id] = t; });
    ...
    var badgeHTML = BADGES.map(function (bd) {
      ...
      return '<div class="bdg' + (on ? ' earned' : '') + '">' + (on ? '' : '<span class="lock"></span>') + '<span class="be">' + icon(bd.ic, 22) + '</span><span class="bn">' + bd.n + '</span></div>';
    }).join('');
  ...
  function showView(name) {
    ...
    else if (name === 'hall') { if (window.renderHall) window.renderHall(); }
