...
document.getElementById('submitBtn').addEventListener('click', async () => {
  ...
  if (!pickedCat) { toast('카테고리부터 골라'); return; }
  if (title.length < 8) { toast('주제는 8자 이상'); return; }
  if (title.length > 120) { toast('주제는 120자까지'); return; }
  if (description.length > 2000) { toast('설명은 2000자까지'); return; }
