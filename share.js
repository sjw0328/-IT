/* ═══════════════════════════════════════════════
   ...
   · 링크 형식: …/d/<토론id>  (경로 기반 — SEO/공유 미리보기용 정적 페이지와 1:1)
     예전 …#d/<id> · …?d=<id> 링크도 thjRouteId() 가 함께 인식한다(하위호환).
   ═══════════════════════════════════════════════ */

'use strict';

/* ── 경로 라우팅 헬퍼 ─────────────────────────────────
   base = 앱이 서비스되는 디렉터리(항상 '/' 로 끝남).
     · 커스텀 도메인 루트 배포      → '/'
     · github.io/<repo> 서브경로    → '/<repo>/'
     · 에디터 미리보기(/app/…)      → '/app/'
   상세 경로(/d/<id>)에서 열려도 base 를 정확히 되돌린다. */
function thjBase() {
  var p = location.pathname;
  var m = p.match(/^(.*?\/)d\/[^/]+\/?$/);   // /…/d/<id> 안에서 열린 경우
  if (m) return m[1];
  if (/\/$/.test(p)) return p;               // 디렉터리(끝이 /)
  return p.replace(/[^/]*$/, '');            // 파일명(index.html) 제거
}
/* 상세 경로(상대) — pushState/replaceState 용 */
function thjPathFor(id) { return thjBase() + 'd/' + encodeURIComponent(id); }
/* 공유용 절대 링크 — 정적 SEO 페이지(/d/<id>/)와 동일 URL */
function thjLinkFor(id) { return location.origin + thjPathFor(id); }
/* 현재 URL 에서 토론 id 추출: 경로(/d/<id>) > 쿼리(?d=<id>) > 예전 해시(#d/<id>) */
function thjRouteId() {
  var m = location.pathname.match(/\/d\/([^/]+)\/?$/);
  if (m) return decodeURIComponent(m[1]);
  var q = new URLSearchParams(location.search).get('d');
  if (q) return q;
  var h = (location.hash || '').match(/^#d\/(.+)$/);
  return h ? h[1] : null;
}
...
window.thjPathFor = thjPathFor;
window.thjBase = thjBase;
window.thjRouteId = thjRouteId;
