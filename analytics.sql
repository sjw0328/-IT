...
--    DAU(오늘)     = 오늘(KST) 접속한 서로 다른 순방문자 수 (presence.sql 의 daily_active)
--    WAU(7일)       = 최근 7일 접속한 서로 다른 순방문자 수
--    ※ daily_active 가 없으면(presence.sql 미재실행) 댓글/투표 작성자 기준으로 자동 폴백.
-- ════════════════════════════════════════════════════════════
...
begin
  ...
  -- daily_active(presence.sql) 가 있으면 순방문자 기준 DAU/WAU → 이게 진짜 "접속자"
  select exists(select 1 from information_schema.tables
    where table_schema='public' and table_name='daily_active') into has_da;
  if has_da then
    dau_basis := 'visitors';
    select count(*)                    into active_today from daily_active where day = kst_today;
    select count(distinct session_key) into active_week  from daily_active where day > kst_today - 7;
  -- 폴백: daily_active 가 없으면(presence.sql 미재실행) 댓글/투표 작성자 distinct 로 추정
  elsif votes_ts then
