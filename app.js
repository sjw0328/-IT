...
function isClosed(t) {
  ...
  if (isNaN(end)) return false;   // 잘못된/빈 날짜 → Invalid Date 비교로 오판하지 않음(마감 여부는 status 로만)
  return end <= Date.now();
}
...
function renderDetailShell(t, commentsLoading) {
  ...
  document.getElementById('detailTop').innerHTML = `
    ...
    <span style="font-size:13px;color:var(--subtext);display:inline-flex;align-items:center;gap:6px">${icon('clock', 14)}${closed ? '' : '종료까지 '}<span class="tnum" style="color:var(--warn);font-weight:800">${t.time || '진행중'}</span></span>${creatorBar(t)}`;
...
/* 작성자에게만 보이는 수명주기 바 — "N일 뒤 삭제" + [7일 연장].
   expires_at 이 있고(=이 규칙 적용 토론) 내가 만든 토론일 때만 렌더링. */
function creatorBar(t) {
  if (!t || !t.expires_at) return '';
  const mine = DB.uid && t.created_by && String(t.created_by) === String(DB.uid);
  if (!mine) return '';
  const left = fmtCountdown(t.expires_at);
  const txt = (left === '종료됨') ? '곧 삭제됩니다' : left + ' 뒤 삭제';
  return `<div class="creator-bar">`
    + `<span class="cb-mine">${icon('user', 12)}내가 연 토론</span>`
    + `<span class="cb-exp">${icon('clock', 12)}${txt}</span>`
    + `<button class="cb-extend" type="button" data-extend="${t.id}">7일 연장</button>`
    + `</div>`;
}
...
function applyVoteUI(side, doVote) {
  if (side && side !== 'a' && side !== 'b') side = null;   // innerHTML 삽입 전 side 화이트리스트 (a/b 외 값 차단)
...
/* ── 초성 검색 ──
   초성 추출은 es-hangul(window.esHangul.getChoseong)을 사용하고,
   CDN 로드 실패 시에만 내장 폴백으로 대체한다.
   "ㅆㄹㄱ"→"쓰레기" 검색 지원. 호환 자모·영문·숫자·공백은 그대로 둬 혼합 입력도 매칭. */
const THJ_CHO = ['ㄱ','ㄲ','ㄴ','ㄷ','ㄸ','ㄹ','ㅁ','ㅂ','ㅃ','ㅅ','ㅆ','ㅇ','ㅈ','ㅉ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'];
function thjChoseong(str) {
  const s = String(str == null ? '' : str);
  const gc = window.esHangul && window.esHangul.getChoseong;
  if (gc) { try { return gc(s); } catch (e) {} }   // es-hangul 우선
  let out = '';                                     // 폴백: 동일 로직 이식
  for (const ch of s) {
    const c = ch.charCodeAt(0);
    if (c >= 0xAC00 && c <= 0xD7A3) out += THJ_CHO[Math.floor((c - 0xAC00) / 588)];
    else out += ch;
  }
  return out;
}
/* 제목/카테고리를 일반 부분일치 또는 초성 일치로 검사 */
function thjMatches(t, q) {
  const lc = q.toLowerCase();
  const title = t.title || '', cat = t.category || '';
  if (title.toLowerCase().includes(lc) || cat.includes(q)) return true;
  const cq = thjChoseong(q);
  return thjChoseong(title).includes(cq) || thjChoseong(cat).includes(cq);
}
...
async function applySearch(q) {
  ...
  let topics = null;
  // 자모(ㄱ-ㅎ, ㅏ-ㅣ)가 섞인 초성 검색은 서버 ilike 로 못 찾으니 곧장 클라이언트 초성 필터.
  const hasJamo = /[\u3131-\u3163]/.test(q);
  if (hasJamo) {
    const all = await DB.topics();
    topics = q[0] === '#'
      ? all.filter(t => (t.category || '').includes(q.slice(1)))
      : all.filter(t => thjMatches(t, q));
  } else {
    // 1순위: 서버 검색 — 전체를 받지 않고 ilike + limit 로 상위 N개만.
    const sres = await DB.searchTopics(q, 48);
    if (sres.ok) topics = sres.rows;
    else {
      // 폴백: 이미 받아둔 목록에서 클라이언트 필터(초성 포함).
      const all = await DB.topics();
      topics = q[0] === '#'
        ? all.filter(t => (t.category || '').includes(q.slice(1)))
        : all.filter(t => thjMatches(t, q));
...
if (_searchEl) {
  // 한글 IME 조합 처리 — 조합 중(ㅆ→쓰→쓰ㄹ…)엔 검색하지 않고, 글자 확정 후에만 검색.
  //   조합 도중 input 이벤트로 미완성 초성/중간 글자("씨리기")로 오검색되는 것을 막는다.
  let _searchTimer = null, _composing = false;
  const runSearch = () => { clearTimeout(_searchTimer); _searchTimer = setTimeout(() => applySearch(_searchEl.value.trim()), 300); };
  _searchEl.addEventListener('compositionstart', () => { _composing = true; });
  _searchEl.addEventListener('compositionend', () => { _composing = false; runSearch(); });
  _searchEl.addEventListener('input', (e) => { if (_composing || e.isComposing) return; runSearch(); });
}
...
(function () {
  ...
  document.addEventListener('click', async (e) => {
    ...
    const r = await DB.extendDebate(id);
    if (r && r.ok) {
      if (CURRENT && String(CURRENT.id) === String(id)) {
        CURRENT.expires_at = r.expires_at;
        if (typeof r.extend_count === 'number') CURRENT.extend_count = r.extend_count;
        refreshDetailCounts(CURRENT);   // 바 다시 그림(버튼 원복 포함)
      }
      thjToast('삭제일을 7일 미뤘어요', 'success', 'clock');
    } else {
      btn.disabled = false;
      btn.textContent = orig;
      const msg = r && r.error === 'not-owner' ? '내가 만든 토론만 연장할 수 있어요'
        : '연장에 실패했어요. 잠시 후 다시 시도해 주세요.';
      thjToast(msg, 'error');
    }
  });
})();
...
refreshMeChip();
renderMain();
/* 사이트링크 검색창(SearchAction) 진입: /?q=검색어 로 들어오면 검색 실행 */
(function initSearchParam() {
  try {
    ...
    if (el) el.value = q;
    if (typeof applySearch === 'function') applySearch(q.trim());
  } catch (e) {}
})();
(function initRoute() {
  const id = (typeof thjRouteId === 'function') ? thjRouteId() : null;
