...
(function () {
  ...
  const KIND = {
    ...
    flip:     { ic: 'arrow-dn', cls: 'flipdn' },
  };

  // 로컬 역전 알림(flip-notify.js) — 서버 알림과 한 목록으로 합친다.
  function localFlips() { try { return (window.THJ_FLIPS && window.THJ_FLIPS.list()) || []; } catch (_) { return []; } }
  function localFlipUnread() { try { return (window.THJ_FLIPS && window.THJ_FLIPS.unread()) || 0; } catch (_) { return 0; } }
  function markLocalRead() { try { window.THJ_FLIPS && window.THJ_FLIPS.markAllRead(); } catch (_) {} }
  ...
  async function renderList() {
    ...
    // 로컬 역전 알림을 병합 → 생성시각 내림차순으로 한 목록
    items = items.concat(localFlips());
    ...
    list.innerHTML = items.map(n => {
      ...
      const ic = n.ic || k.ic, cls = n.cls || k.cls;
      ...
      return `<div class="notif-item ${n.read ? '' : 'unread'}" ${n.debateId ? `data-did="${esc(n.debateId)}"` : ''}>
        <div class="ni-ic ${cls}">${icon(ic, 17)}</div>
        <div class="ni-tx">
  ...
  async function openPanel() {
    ...
    markTimer = setTimeout(async () => { await DB.markAllRead(); markLocalRead(); refreshBadge(); }, 900);
  }
  function closePanel() {
    if (!panel) return;
  ...
  /* ── 시작: 배지 + 폴링 + 실시간 ── */
  refreshBadge();
  // 45초 → 5분: 새 알림은 아래 Realtime 구독이 즉시 밀어주므로 폴링은 보정용으로만 (요청 1/6 이하)
  setInterval(() => { if (!document.hidden) refreshBadge(); }, 300000);
  ...
  // 로컬 역전 알림이 갱신되면 배지/목록 즉시 반영
  document.addEventListener('thj-flips', () => { refreshBadge(); if (open) renderList(); });
})();
