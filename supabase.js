...
const DB = {
  ...
  async reports() {
    ...
    if (!g.missing) {
      ...
      return (g.data || []).map(r => ({
        id: r.id, commentId: r.comment_id, targetUserId: r.target_user_id || null,
        reason: r.reason, status: r.status || 'new', blinded: !!r.is_blinded,
        reporter: r.reporter_nick || '익명',
        created_at: r.created_at,
        created_label: r.created_at ? (typeof relTime === 'function' ? relTime(r.created_at) : '') : '',
  ...
  presence(scope, onCount) {
    ...
    let stopped = false, timer = null, channel = null, fellBack = false;
    ...
    // 60초마다: 서버에 하트비트 송신(수신·저장) + 현재 인원수 수신
    const beat = async () => {
    ...
    beat();                              // 즉시 1회
    timer = setInterval(beat, 60000);    // 이후 60초마다
