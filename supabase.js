...
function genNick() {
  ...
  return adj[rand(adj.length)] + ani[rand(ani.length)];
}
function myNick() {
...
function mapComment(c, myLikes) {
  ...
  return {
    id: c.id, side, nickname: nick, emoji: c.emoji || emojiFor(nick),
...
/* ── 데이터 접근 계층 (전부 서버 구동) ───────── */
const DB = {
  live: !!sb, ratio, voters, myNick, genNick, emojiFor,
  lastError: null,
  ...
  async vote(topicId, side) {
    ...
    if (error) console.error('[toronhaja] 투표 실패:', error.message);
    else invalidateTopics();   // 피드 카드 비율 갱신
  },
  ...
  async banUser(userId, reason) {
    ...
    const { error } = await sb.from('users').update({ is_banned: true, ban_reason: reason || '관리자 제재' }).eq('id', userId);
    if (error) { console.error('[toronhaja] 제재 실패 (users_admin_update 정책 필요):', error.message); return { error: error.message }; }
    return { ok: true };
  },
  ...
  async myBanStatus() {
    ...
    const { data } = await sb.from('users').select('is_banned, ban_reason').eq('id', MY_UID).single();
    return { banned: !!(data && data.is_banned), reason: (data && data.ban_reason) || '이용 약관 위반' };
  },
  ...
  async deleteDebate(id) {
    ...
    if (error) { console.error('[toronhaja] 토론 삭제 실패 (debates_delete 정책 필요):', error.message); return { error: error.message }; }
    return { ok: true };
  },
    ...
    return sb.channel('rt-c-' + debateId)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments', filter: 'debate_id=eq.' + debateId },
        async ({ new: row }) => {
          ...
          onInsert({ id: row.id, side: String(row.side).toLowerCase(), nickname: nick, emoji: emojiFor(nick), body: row.content, likes: 0, created_at: '방금 전', mine: row.user_id === MY_UID });
        })
      .subscribe();
  },
