...
let _FEEDCOLS = 'id,category,title,a_count,b_count,comment_count,vote_count,is_hot,ends_at,status,created_at,side_a_label,side_b_label,creator_nick';
...
const DB = {
  ...
  async addTopic({ category, title, description }) {
    ...
    const base = { title, description, side_a_label: '찬성', side_b_label: '반대', creator_nick: myNick(), ends_at };
  ...
  async hallOfFame(force) {
    ...
    try { localStorage.setItem('thj_hall', JSON.stringify({ at: Date.now(), data })); } catch (e) {}
    return data;
  },
