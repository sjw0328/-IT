...
/* 아바타 — 이모지 대신 닉네임 머리글자(모노그램). 에디토리얼 톤. */
function emojiFor(nick) { const s = String(nick == null ? '' : nick).trim(); return (s[0] || '?').toUpperCase(); }
...
/* 댓글/투표 시 실제 users 행 생성 (최초 1회). 단순 열람으로는 생성하지 않음.
   ── 동시 호출 방지: 진행 중인 insert 를 하나로 합쳐(_ensuringUser) 같은 기기가
      순식간에 여러 users 레코드를 만드는 것을 막는다(대량 레코드 생성/쿼터 고갈 방지). */
let _ensuringUser = null;
async function ensureUser() {
  if (MY_UID) return MY_UID;
  if (!sb) return null;
  if (_ensuringUser) return _ensuringUser;   // 이미 생성 중 → 같은 promise 재사용
  _ensuringUser = (async () => {
    const nick = myNick();
  ...
  try { return await _ensuringUser; }
  finally { _ensuringUser = null; }   // 성공 시 MY_UID 가드가 잡고, 실패 시 재시도 허용
}
...
function mapDebate(d) {
  return {
    ...
    creator_nick: d.creator_nick || null,
    created_by: d.created_by || null, expires_at: d.expires_at || null, extend_count: +d.extend_count || 0,
  };
}
...
const TOPICS_TTL = 60000;   // 60초 — 홈탭 재요청 최소화 (투표는 patchVote 로즉시 보정되므로 체감 지연 없음)
function invalidateTopics() { _topicsCache = null; _topicsAt = 0; }

/* optimize.sql 미실행(집계 컬럼 없음) 시 임시 집계 폴백 — 예전 방식 */
/* ── 목록 전송량 최소화 ──────────────────────────────
   피드/검색/랭킹 카드는 description(본문 전문) 같은 대용량 컬럼을 안 쓴다.
   카드에 실제 쓰는 컬럼만 골라 받으면 행당 전송량이 크게 줄어든다(수천~수만 행에서 체감).
   optional 컬럼(side_a_label·creator_nick 등)이 없는 스키마에선 에러 → '*' 로 1회 폴백하고
   그 세션 동안은 계속 '*' 를 써서 재실패를 막는다. (상세 topic()은 본문이 필요해 '*' 유지) */
const _FEEDCOLS_ORIG = 'id,category,title,a_count,b_count,comment_count,vote_count,is_hot,ends_at,status,created_at,side_a_label,side_b_label,creator_nick';
let _FEEDCOLS = _FEEDCOLS_ORIG;
let _feedColsFellBackAt = 0;
const _FEEDCOLS_RETRY = 300000;   // 5분
async function runFeedQuery(build) {
  // '*' 폴백이 오래되면 원래 컬럼 목록으로 복구 시도 — 스키마가 나중에 정상화돼도 계속 전체 컬럼을 받지 않도록.
  if (_FEEDCOLS === '*' && _feedColsFellBackAt && (Date.now() - _feedColsFellBackAt) > _FEEDCOLS_RETRY) {
    _FEEDCOLS = _FEEDCOLS_ORIG; _feedColsFellBackAt = 0;
  }
  let res;
  try { res = await build(_FEEDCOLS); } catch (e) { res = { error: e }; }
  if (res && res.error && _FEEDCOLS !== '*' &&
      /column|schema|does not exist|42703|unknown|select/i.test(res.error.message || '')) {
    _FEEDCOLS = '*'; _feedColsFellBackAt = Date.now();
    try { res = await build('*'); } catch (e) { res = { error: e }; }
  }
  return res;
}
...
/* 집계 컬럼이 0 인 토론만 votes/comments 를 한 쿼리로 세어 정확히 보정한다.
   집계가 살아있는(>0) 토론은 건드리지 않아 추가 전송이 거의 없다.
   topics()(전체) · topicsPage()(페이지) · ranking()(상위 N) 이 공유.
   ⚠ 인자 배열을 제자리(in-place) 수정한다. */
async function correctZeroCounts(mapped) {
  if (!sb || !mapped || !mapped.length) return mapped;
  // 세션 내 재검사 금지 — 진짜 0표 토론이 매 피드 로드마다 보정 쿼리를 재유발하는 것을 막는다
  if (!correctZeroCounts._done) correctZeroCounts._done = new Set();
  const seen = correctZeroCounts._done;
  if (seen.size > 4000) seen.clear();   // 장시간 이용 시 Set 무한 증가(메모리 누수) 방지 — 상한 초과 시 초기화
  try {
    const zeroIds = mapped.filter(t => (t.a_count + t.b_count) === 0 && !seen.has('v' + t.id)).map(t => t.id).slice(0, 80);
    zeroIds.forEach(id => seen.add('v' + id));
    if (zeroIds.length) {
      ...
      if (vrows && vrows.length) {
        ...
        mapped.forEach(t => { const g = agg[t.id]; if (g) { t.a_count = g.a; t.b_count = g.b; } });
      }
    }
  } catch (e) { /* 보정 실패해도 피드는 그대로 표시 */ }
  try {
    const zeroCIds = mapped.filter(t => t.comment_count === 0 && !seen.has('c' + t.id)).map(t => t.id).slice(0, 80);
    zeroCIds.forEach(id => seen.add('c' + id));
    if (zeroCIds.length) {
      ...
      if (crows && crows.length) {
        ...
        mapped.forEach(t => { const n = cagg[t.id]; if (n) t.comment_count = n; });
      }
    }
  } catch (e) { /* 보정 실패해도 피드는 그대로 표시 */ }
  return mapped;
...
/* 상세 즉시 진입용 스냅샷(localStorage) 갱신 — 받은 토론만 병합 저장.
   서버 왕복 없이 상세 셸을 0.1초 안에 그릴 수 있게 한다(딥링크/새로고침 커버). */
function mergeSnapshot(mapped) {
  try {
    const snap = JSON.parse(localStorage.getItem('thj_topic_snap') || '{}');
    mapped.forEach(t => { snap[t.id] = {
      ...
      side_a_label: t.side_a_label, side_b_label: t.side_b_label, creator_nick: t.creator_nick,
    }; });
...
const DB = {
  ...
  async topics(force) {
    ...
    let data, error;
    // 안전 상한 500개 — 이 경로는 폴백용이라 데이터가 아무리 많아도 전체를 내려받지 않는다.
    try { ({ data, error } = await runFeedQuery((cols) => sb.from('debates').select(cols).order('created_at', { ascending: false }).limit(500))); }
    catch (e) { error = e; }
    if (error) {
    ...
    // 집계 컬럼이 0 으로 굳은 토론만 votes/comments 를 한 쿼리로 세어 정확히 보정.
    await correctZeroCounts(_topicsCache);
    _topicsAt = Date.now();
  ...
  async ranking(n = 5, force) {
    if (!sb) return [];
    // 5분 localStorage 캐시 — 홈 진입/탭 전환마다 랭킹 쿼리를 다시 쏘지 않는다
    try {
      const c = JSON.parse(localStorage.getItem('thj_rank_cache') || 'null');
      if (!force && c && (Date.now() - c.at) < 300000 && Array.isArray(c.rows) && c.rows.length >= n) {
        return c.rows.slice(0, n).map(mapDebate);   // 재매핑 → 남은 시간(countdown) 재계산
      }
    } catch (e) {}
    const lim = Math.max(n, 8);   // 홈(5)·전당(8)이 캐시 하나를 공유하도록 넓게 받는다
    // 전체를 받지 않고 vote_count(=찬+반) 내림차순 상위 lim개만 가져온다(사이드바 전용 쿼리).
    try {
      const { data, error } = await runFeedQuery((cols) => sb.from('debates')
        .select(cols).order('vote_count', { ascending: false }).order('created_at', { ascending: false }).limit(lim));
      if (!error && data) {
        const mapped = data.map(mapDebate);
        await correctZeroCounts(mapped);
        mergeSnapshot(mapped);
        try { localStorage.setItem('thj_rank_cache', JSON.stringify({ at: Date.now(), rows: data })); } catch (e) {}
        return mapped.slice(0, n);
      }
    } catch (e) { /* vote_count 컬럼 미존재(paginate.sql 전) → 폴백 */ }
    // 폴백: 기존 방식(전체 받아 클라이언트 정렬)
    const list = await this.topics();
    return [...list].sort((x, y) => voters(y) - voters(x)).slice(0, n);
  },

  /* 홈 피드 서버 페이지네이션 — 정렬별 order + range + count.
     · new: created_at desc   · hot(기본): is_hot, comment_count, created_at desc
     · pop: vote_count, created_at desc
     · close: closeness asc (scale.sql 필요 — 미실행 시 컴럼 없음 → ok:false 폴백)
     실패(vote_count 미존재·optimize.sql 전 등) 시 { ok:false } → 호출부가 전체 경로로 폴백. */
  async topicsPage({ offset = 0, limit = 12, sort = 'hot' } = {}) {
    if (!sb) return { ok: false, rows: [], total: 0 };
    // count 는 첫 페이지에서만, 그것도 'planned'(플래너 추정)로 — 1억 행에서 매 요청 풀스캔을 피한다.
    const wantCount = offset === 0;
    const res = await runFeedQuery((cols) => {
      let q = sb.from('debates').select(cols, wantCount ? { count: 'planned' } : undefined);
      if (sort === 'new')        q = q.order('created_at', { ascending: false });
      else if (sort === 'pop')   q = q.order('vote_count', { ascending: false }).order('created_at', { ascending: false });
      else if (sort === 'close') q = q.order('closeness', { ascending: true }).order('vote_count', { ascending: false }).order('created_at', { ascending: false });
      else                       q = q.order('is_hot', { ascending: false }).order('comment_count', { ascending: false }).order('created_at', { ascending: false });
      return q.range(offset, offset + limit - 1);
    });
    if (!res || res.error) {
      if (res && res.error) console.warn('[toronhaja] topicsPage 실패 → 전체 경로로 폴백:', res.error.message);
      return { ok: false, rows: [], total: 0 };
    }
    const rows = res.data || [];
    if (rows.length && rows[0].a_count === undefined) return { ok: false, rows: [], total: 0 }; // optimize.sql 전 → 폴백
    const mapped = rows.map(mapDebate);
    await correctZeroCounts(mapped);
    mergeSnapshot(mapped);
    DB.lastError = null;
    return { ok: true, rows: mapped, total: res.count || 0 };
  },

  /* 서버 검색 — 전체를 받지 않고 ilike + limit 로 상위 N개만.
     '#카테고리' 는 category 필터. 실패 시 { ok:false } → 호출부가 클라이언트 필터로 폴백. */
  async searchTopics(q, limit = 48) {
    if (!sb || !q) return { ok: false, rows: [] };
    const safe = String(q).replace(/[%,()\\]/g, '').trim();   // PostgREST or() 문법 보호
    if (!safe) return { ok: false, rows: [] };
    try {
      const res = await runFeedQuery((cols) => {
        let query = sb.from('debates').select(cols);
        if (safe[0] === '#') query = query.ilike('category', `%${safe.slice(1)}%`);
        else query = query.or(`title.ilike.%${safe}%,category.ilike.%${safe}%`);
        return query.order('created_at', { ascending: false }).limit(limit);
      });
      const { data, error } = res || {};
      if (error) { console.warn('[toronhaja] 서버 검색 실패 → 클라이언트 폴백:', error.message); return { ok: false, rows: [] }; }
      const mapped = (data || []).map(mapDebate);
      await correctZeroCounts(mapped);
      mergeSnapshot(mapped);
  ...
  async comments(debateId, side, offset = 0, limit = 10) {
    if (!sb) return { rows: [], total: 0 };
    let q = sb.from('comments').select('*, users(nickname), comment_likes(count)', { count: 'exact' }).eq('debate_id', debateId);
    if (side) q = q.eq('side', side.toUpperCase());
    q = q.order('created_at', { ascending: false }).range(offset, offset + limit - 1);
    const { data, error, count } = await q;
    if (error) { console.error('[toronhaja] 댓글 조회 실패:', error.message); return { rows: [], total: 0 }; }
    const myLikes = await this.myLikes(debateId);
    return { rows: (data || []).map(c => mapComment(c, myLikes)), total: count || 0 };
  },
  ...
  async addTopic({ category, title, description }) {
    if (!sb) return null;
    const uid = await ensureUser();
    // 인앱 생성 토론에 7일 시간 제한을 건다(로직·카운트다운은 기존 ends_at 기반).
    const ends_at = new Date(Date.now() + 7 * 86400000).toISOString();
    // 생성 14일 뒤 자동 삭제 기준시각(작성자가 상세에서 7일씩 연장 가능 — lifecycle.sql).
    const expires_at = new Date(Date.now() + 14 * 86400000).toISOString();
    const base = { title, description, side_a_label: '찬성', side_b_label: '반대', creator_nick: myNick(), ends_at, expires_at };
    if (uid) base.created_by = uid;
    invalidateTopics();
    let res = await sb.from('debates').insert({ ...base, category }).select().single();
    if (res.error && /ends_at/i.test(res.error.message)) {
      // ends_at 컬럼이 없는 스키마 → 그 필드만 빼고 재시도
      delete base.ends_at;
      res = await sb.from('debates').insert({ ...base, category }).select().single();
    }
    if (res.error && /expires_at/i.test(res.error.message)) {
      // expires_at 컬럼이 없는 스키마(lifecycle.sql 미실행) → 그 필드만 빼고 재시도
    ...
    if (res.error && /creator_nick/i.test(res.error.message)) {
      // creator-attribution.sql 미실행(컬럼 없음) → 그 필드만 빼고 재시도
      delete base.creator_nick;
      res = await sb.from('debates').insert({ ...base, category }).select().single();
      if (res.error && /category/i.test(res.error.message)) {
        res = await sb.from('debates').insert(base).select().single();
      }
  ...
  /* 작성자 연장 — 삭제 예정일을 7일 미룬다(lifecycle.sql 의 thj_extend_debate).
     RPC 우선 → 미설치 시 직접 update 폴백(created_by 가드). */
  async extendDebate(id) {
    if (!sb) return { error: 'no client' };
    const uid = MY_UID;
    if (!uid) return { error: 'no-user' };
    invalidateTopics();
    const { data, error } = await sb.rpc('thj_extend_debate', { did: id, uid });
    if (!error && data) {
      if (typeof data === 'object') {
        if (data.ok) return { ok: true, expires_at: data.expires_at, extend_count: data.extend_count };
        return { error: data.error || 'failed' };
      }
      if (typeof data === 'string') return { ok: true, expires_at: data };
    }
    if (error && !rpcMissing(error)) { console.error('[toronhaja] 연장 실패:', error.message); return { error: error.message }; }
    // 폴백 (lifecycle.sql 미실행): 직접 update — debates_update 정책 + created_by 필요
    const { data: row } = await sb.from('debates').select('expires_at, created_by').eq('id', id).single();
    if (!row) return { error: 'not-found' };
    if (String(row.created_by || '') !== String(uid)) return { error: 'not-owner' };
    const baseT = (row.expires_at && new Date(row.expires_at) > new Date()) ? new Date(row.expires_at) : new Date();
    const next = new Date(baseT.getTime() + 7 * 86400000).toISOString();
    const { error: uErr } = await sb.from('debates').update({ expires_at: next }).eq('id', id).eq('created_by', uid);
    if (uErr) { console.error('[toronhaja] 연장 실패(직접):', uErr.message); return { error: uErr.message }; }
  ...
  async latestAnnouncement() {
    if (!sb) return null;
    // 10분 localStorage 캐시 — 공지는 거의 안 바뀌는데 홈 진입마다 조회하지 않는다 (null 결과도 캐시)
    try {
      const c = JSON.parse(localStorage.getItem('thj_ann_cache') || 'null');
    ...
    try { localStorage.setItem('thj_ann_cache', JSON.stringify({ at: Date.now(), data: row })); } catch (e) {}
    return row;
  },
  async announcements() {
    if (!sb) return [];
  ...
  async addAnnouncement(body) {
    if (!sb) return { error: 'no client' };
    try { localStorage.removeItem('thj_ann_cache'); localStorage.removeItem('thj_annlist_cache'); } catch (e) {}
  ...
  async adminStats() {
    ...
    const [tp, rp, cm, bn] = await Promise.all([
      sb.from('debates').select('id', { count: 'planned', head: true }).neq('status', 'closed'),
      sb.from('reports').select('id', { count: 'planned', head: true }),
      sb.from('comments').select('id', { count: 'planned', head: true }),
      sb.from('users').select('id', { count: 'planned', head: true }).eq('is_banned', true),
    ]);
  ...
  /* ── 사용자 목록 (관리자) ──────────────────────
     검색·정지필터·댓글수 포함. admin-extra.sql 의 게이트 RPC 우선 →
     미설치 시 직접 조회로 폴백(읽기 정책이 열려 있으면 동작). */
  async usersList(opts) {
    if (!sb) return { rows: [], error: 'no client' };
    const o = opts || {};
    const search = (o.search || '').trim();
    const bannedOnly = !!o.bannedOnly;
    const limit = o.limit || 200;
    const map = (u) => ({
      id: u.id, nickname: u.nickname || '익명',
      emoji: emojiFor(u.id || u.nickname), tag: tagOf(u.id || u.nickname),
      is_banned: !!u.is_banned, ban_reason: u.ban_reason || null,
      banned_until: u.banned_until || null, created_at: u.created_at,
      comments: +u.comments || 0,
    });
    // 게이트 RPC 우선 (admin-extra.sql)
    const g = await adminRPC('thj_admin_users', { search: search || null, banned_only: bannedOnly, lim: limit });
    if (!g.missing) {
      if (g.error) { console.error('[toronhaja] 유저 목록 실패:', g.error.message); return { rows: [], error: g.error.message }; }
      return { rows: (g.data || []).map(map) };
    }
    // 폴백: 직접 조회
    const cols = 'id, nickname, is_banned, ban_reason, banned_until, created_at';
    let q = sb.from('users').select(cols).order('created_at', { ascending: false }).limit(limit);
    if (bannedOnly) q = q.eq('is_banned', true);
    if (search) q = q.ilike('nickname', '%' + search + '%');
    let { data, error } = await q;
    if (error && /banned_until|column|schema/i.test(error.message || '')) {
      let q2 = sb.from('users').select('id, nickname, is_banned, ban_reason, created_at').order('created_at', { ascending: false }).limit(limit);
      if (bannedOnly) q2 = q2.eq('is_banned', true);
      if (search) q2 = q2.ilike('nickname', '%' + search + '%');
      ({ data, error } = await q2);
    }
    if (error) { console.error('[toronhaja] 유저 목록 실패:', error.message); return { rows: [], error: error.message }; }
    const rows = data || [];
    // 댓글 수 집계 (comments 는 작음 → 가벼움)
    const counts = {};
    try {
      const ids = rows.map(u => u.id).filter(Boolean);
      if (ids.length) {
        const { data: cs } = await sb.from('comments').select('user_id').in('user_id', ids);
        (cs || []).forEach(c => { if (c.user_id) counts[c.user_id] = (counts[c.user_id] || 0) + 1; });
      }
    } catch (e) { /* 집계 실패해도 목록은 표시 */ }
    return { rows: rows.map(u => map(Object.assign({}, u, { comments: counts[u.id] || 0 }))) };
  },

  /* 토론 내용 수정 (제목·카테고리·설명) — 게이트 RPC 우선, 없으면 직접 update 폴백 */
  async updateDebate(id, fields) {
    ...
    const g = await adminRPC('thj_admin_update_debate', {
      did: id,
      new_title: patch.title != null ? patch.title : null,
      new_category: patch.category != null ? patch.category : null,
      new_description: patch.description != null ? patch.description : null,
    });
    if (!g.missing) {
      if (g.error) { console.error('[toronhaja] 토론 수정 실패:', g.error.message); return { error: g.error.message }; }
      return { ok: true };
    }
    let res = await sb.from('debates').update(patch).eq('id', id);
    if (res.error && /category/i.test(res.error.message || '')) {
      const rest = Object.assign({}, patch); delete rest.category;
      res = await sb.from('debates').update(rest).eq('id', id);
    }
    if (res.error) { console.error('[toronhaja] 토론 수정 실패 (debates_update 정책 필요):', res.error.message); return { error: res.error.message }; }
    return { ok: true };
  },

  /* ── 분석용 시계열 (관리자 대시보드/분석 탭) ──
     최근 N일의 댓글·투표·신규유저·신규토론을 일자별로 버킷팅 + 시간대(0~23시) 댓글 분포.
     테이블이 작아(읽기 한정) 가볍게 동작. votes.created_at 없으면 투표 시리즈는 0. */
  async analyticsSeries(days) {
    if (!sb) return null;
    const N = days || 14;
    const since = new Date(); since.setHours(0, 0, 0, 0);
    since.setTime(since.getTime() - (N - 1) * 86400000);
    const sinceIso = since.toISOString();
    const dayKey = (iso) => { const d = new Date(iso); d.setHours(0, 0, 0, 0); return d.getTime(); };
    const buckets = {}; const labels = [];
    for (let i = 0; i < N; i++) { const d = new Date(since.getTime() + i * 86400000); buckets[d.getTime()] = { comments: 0, votes: 0, users: 0, debates: 0 }; labels.push(d); }
    const hours = new Array(24).fill(0);
    const grab = async (table) => { try { const { data, error } = await sb.from(table).select('created_at').gte('created_at', sinceIso); if (error) return null; return data || []; } catch (e) { return null; } };
    const [cs, ds, us] = await Promise.all([grab('comments'), grab('debates'), grab('users')]);
    const vs = await grab('votes');
    const votesHasTs = vs !== null;
    (cs || []).forEach(r => { const k = dayKey(r.created_at); if (buckets[k]) buckets[k].comments++; const h = new Date(r.created_at).getHours(); hours[h]++; });
    (ds || []).forEach(r => { const k = dayKey(r.created_at); if (buckets[k]) buckets[k].debates++; });
    (us || []).forEach(r => { const k = dayKey(r.created_at); if (buckets[k]) buckets[k].users++; });
    (vs || []).forEach(r => { const k = dayKey(r.created_at); if (buckets[k]) buckets[k].votes++; });
    const series = labels.map(d => { const b = buckets[d.getTime()]; return { t: d.getTime(), label: (d.getMonth() + 1) + '/' + d.getDate(), comments: b.comments, votes: b.votes, users: b.users, debates: b.debates }; });
    return { days: series, votesHasTs, hours };
  },

  /* ── 전체 댓글 목록 (관리자 댓글 모더레이션) ──
     작성자 닉네임 + 토론 제목 포함, 최신순. filter: ''|'blind'|'reported'. */
  async allComments(opts) {
    if (!sb) return { rows: [], error: 'no client' };
    const o = opts || {}; const limit = o.limit || 200;
    const sel = 'id, content, side, is_blinded, report_count, created_at, user_id, users(nickname), debates(title)';
    let q = sb.from('comments').select(sel).order('created_at', { ascending: false }).limit(limit);
    if (o.filter === 'blind') q = q.eq('is_blinded', true);
    else if (o.filter === 'reported') q = q.gt('report_count', 0);
    let { data, error } = await q;
    if (error && /report_count|column|schema/i.test(error.message || '')) {
      const sel2 = 'id, content, side, is_blinded, created_at, user_id, users(nickname), debates(title)';
      let q2 = sb.from('comments').select(sel2).order('created_at', { ascending: false }).limit(limit);
      if (o.filter === 'blind') q2 = q2.eq('is_blinded', true);
      ({ data, error } = await q2);
    }
    if (error) { console.error('[toronhaja] 댓글 목록 실패:', error.message); return { rows: [], error: error.message }; }
    let rows = (data || []).map(c => ({
      id: c.id, content: c.content || '', side: String(c.side || 'A').toLowerCase(),
      nickname: (c.users && c.users.nickname) || '익명', uid: c.user_id || null,
      emoji: emojiFor(c.user_id || ((c.users && c.users.nickname) || '익명')),
      debate: (c.debates && c.debates.title) || '—',
      blinded: !!c.is_blinded, reports: +c.report_count || 0, created_at: c.created_at,
    }));
    if (o.search) { const s = o.search.toLowerCase(); rows = rows.filter(r => r.content.toLowerCase().includes(s) || r.nickname.toLowerCase().includes(s) || r.debate.toLowerCase().includes(s)); }
    return { rows };
  },
  ...
  presence(scope, onCount) {
    ...
    // 페이지 이탈(새로고침·이동·앱 전환) 시에도 접속 해제를 서버에 알리고 타이머를 정리 → 메모리 누수·유령 접속자 방지
    const onLeave = () => stop();
  ...
  /* ── 베스트 의견 (개념글) — sql/best.sql 의 like_count 컬럼 기반 ──
     토론당 소형 쿼리 1개(공감 3+ 상위 몇 행)로 각 진영 상단에 고정할 의견을 가져온다.
     컬럼 미존재(best.sql 미실행) 시 null 반환 → 호출부가 이미 받은 첫 페이지에서
     클라이언트 선정으로 폴백(추가 요청 0). */
  async bestComments(debateId, minLikes = 3, limit = 8) {
    if (!sb) return null;
    const { data, error } = await sb.from('comments')
      .select('*, users(nickname)')
      .eq('debate_id', debateId).eq('is_blinded', false)
      .gte('like_count', minLikes)
      .order('like_count', { ascending: false }).limit(limit);
    if (error) {
      if (/like_count|column|schema/i.test(error.message || '')) return null;   // best.sql 미실행
      console.warn('[toronhaja] 베스트 의견 조회 실패:', error.message);
      return [];
    }
    return (data || []).map(c => {
      const m = mapComment({ ...c, likes: +c.like_count || 0 }, null);
      m.best = true;
      return m;
    });
  },

  /* ── 명예의 전당 — 10분 localStorage 캐시로 재방문 서버 비용 0 ──
     구성: 레전드 토론(기존 ranking 쿼리 재사용) + 명언(공감 상위 댓글 1쿼리)
     + 논객 랭킹(명언 행을 클라이언트에서 집계 — 추가 쿼리 0). */
  async hallOfFame(force) {
    if (!sb) return null;
    const TTL = 600000;   // 10분
    if (!force) {
      try {
        const c = JSON.parse(localStorage.getItem('thj_hall') || 'null');
        if (c && (Date.now() - c.at) < TTL && c.data) return c.data;
      } catch (e) {}
    }
    const legends = (await this.ranking(8)).filter(t => voters(t) > 0);
    let quotes = [], speakers = [], noBest = false;
    const res = await sb.from('comments')
      .select('id, debate_id, side, content, like_count, user_id, created_at, users(nickname), debates(title)')
      .eq('is_blinded', false).gt('like_count', 0)
      .order('like_count', { ascending: false }).limit(30);
    if (res.error) {
      if (/like_count|column|schema/i.test(res.error.message || '')) noBest = true;   // best.sql 미실행
      else console.warn('[toronhaja] 명언 조회 실패:', res.error.message);
    } else {
      quotes = (res.data || []).map(c => ({
        id: c.id, debateId: c.debate_id, side: String(c.side || 'A').toLowerCase(),
        body: c.content || '', likes: +c.like_count || 0,
        nickname: (c.users && c.users.nickname) || '익명',
        uid: c.user_id || null,
        tag: tagOf(c.user_id || ((c.users && c.users.nickname) || '익명')),
        debateTitle: (c.debates && c.debates.title) || '',
      }));
      // 논객 랭킹 — 명언 행 집계 (추가 쿼리 0)
      const agg = {};
      quotes.forEach(q => {
        const k = q.uid || q.nickname;
        (agg[k] = agg[k] || { nickname: q.nickname, tag: q.tag, likes: 0, quotes: 0 });
        agg[k].likes += q.likes; agg[k].quotes++;
      });
      speakers = Object.values(agg).sort((x, y) => y.likes - x.likes).slice(0, 8);
    }
    const data = { legends, quotes: quotes.slice(0, 12), speakers, noBest };
    try {
      const payload = JSON.stringify({ at: Date.now(), data });
      if (payload.length < 512000) localStorage.setItem('thj_hall', payload);   // ~0.5MB 상한 — 5MB localStorage 쿼터 초과 방지
    } catch (e) {}
    return data;
  },
  ...
  /* 활성 공지 목록 10분 캐시 — 벨 배지 폴링·패널이 공유 (공지 등록/토글 시 무효화) */
  async _activeAnns(limit) {
    if (!sb) return [];
    try {
      const c = JSON.parse(localStorage.getItem('thj_annlist_cache') || 'null');
    ...
    const rows = data || [];
    try { localStorage.setItem('thj_annlist_cache', JSON.stringify({ at: Date.now(), rows })); } catch (e) {}
    return rows;
  },
  ...
  async notifications(limit = 30) {
    ...
    const anns = await this._activeAnns(10);   // 10분 캐시 — 서버 요청 절약
    ...
    out.sort((x, y) => (new Date(y.created_at) - new Date(x.created_at)) || String(y.id).localeCompare(String(x.id)));
  ...
  async unreadCount() {
    ...
    const seen = +localStorage.getItem('thj_notif_seen') || 0;
    const anns = await this._activeAnns(10);   // 10분 캐시 — 배지 폴링이 공지 테이블을 때리지 않는다
    (anns || []).forEach(a => { if (new Date(a.created_at).getTime() > seen) n++; });
    return n;
  },
  async markAllRead() {
