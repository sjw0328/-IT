...
function _nickEditorBody(current, emoji, { title, sub, cta }) {
  return `
    ...
    <div class="ob-nick">
      <input id="obNick" type="text" maxlength="16" value="${_esc(current)}" autocomplete="off" spellcheck="false" />
...
function thjOnboard() {
  ...
  const steps = [
    ['scale', '입장을 고른다', '찬성 · 반대 중 한쪽을 선택'],
    ['comment', '근거를 남긴다', '같은 편끼리 의견이 모임'],
    ['chart', '실시간 결과', '여론이 즉시 % 로 갈린다'],
  ].map(([ic, t, d]) => `
  ...
  const { ov, close } = thjModal(`
    ...
    ${_nickEditorBody(nick, emoji, {
      title: '닉네임 정하기',
      sub: '익명으로 참여해요. 마음에 드는 이름을 고르거나 주사위를 굴려보세요.',
      cta: '토론 시작하기',
    })}`, { persistent: true });
