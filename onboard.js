...
function thjModal(innerHTML, opts = {}) {
  ...
  ov.innerHTML = `<div class="thj-modal" role="dialog" aria-modal="true" aria-label="${_esc(opts.label || '토론하자 안내')}">${innerHTML}</div>`;
...
function _nickEditorBody(current, emoji, { title, sub, cta }) {
  return `
    ...
    <div class="ob-nick">
      <input id="obNick" type="text" maxlength="10" value="${_esc(current)}" autocomplete="off" spellcheck="false" aria-label="닉네임" />
